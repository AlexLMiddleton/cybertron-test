...
export default function() {
  ...
  const exercise2 = _ => {
    return cars.filter(car => car.year >= 2001 && car.year <= 2004)
  }
  ...
  const ex4 =
    'Use map, filter and reduce with compose to return the price of the the most'
  ;('expensive car from the 2000s ')
  const exercise4 = _ => {
    return compose(
      reduce((acc, car) => (car > acc ? car : acc), ''),
  ...
  const ex6 = `Use compose and filter to return cars with V8s, map over them and return the
  names of the cars with the first letter capitalized.`
