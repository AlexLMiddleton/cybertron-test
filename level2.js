...
export default function() {
  ...
  const ex4 = `Use map, filter and reduce with compose to return the
  full name (as a string) of the female from Wyoming. `
  const exercise4 = _ => {
    return compose(
      reduce(
        (acc, client) => `${client.name.firstName} ${client.name.lastName}`,
        ''
      ),
      filter(
        client =>
